...
const UpdateBook = (props: IProps) => {
    const { dataUpdate, isOpenUpdate, setDataUpdate, setIsOpenUpdate,
        reloadTableBook
    } = props;
    console.log("check data update: ", dataUpdate?.slider)
    ...
    useEffect(() => {
        ...
        if (dataUpdate) {
            ...
            const arrSlider: UploadFile[] = dataUpdate.slider.map(item => {
                return {
                    uid: uuidv4(),
                    name: item,
                    status: 'done',
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = async (values) => {
        ...
        const res = await updateBookAPI(values._id, fileListThumbnail[0].name, sliderBook, values.mainText, values.author, values.price, values.quantity, values.category);
        if (res.data) {
